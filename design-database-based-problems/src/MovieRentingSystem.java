...
class MovieRentingSystem {
    ...
    // Return up to 5 rented copies [shop, movie]
...
class Main4 {
    public static void main(String[] args) {
        ...
        MovieRentingSystem mrs = null;

        for (int i = 0; i < commands.length; i++) {
            switch (commands[i]) {
                case "MovieRentingSystem":
                    int n = (int) arguments[i][0];
                    int[][] entries = (int[][]) arguments[i][1];
                    mrs = new MovieRentingSystem(n, entries);
                    output.add(null);
                    break;
                case "search":
                    output.add(mrs.search((int) arguments[i][0]));
                    break;
                case "rent":
                    mrs.rent((int) arguments[i][0], (int) arguments[i][1]);
                    output.add(null);
                    break;
                case "drop":
                    mrs.drop((int) arguments[i][0], (int) arguments[i][1]);
                    output.add(null);
                    break;
                case "report":
                    output.add(mrs.report());
                    break;
            }
        }
